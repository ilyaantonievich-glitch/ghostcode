...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const withProjectEditRules = Effect.fn("SessionPrompt.withProjectEditRules")(function* (
      req: Parameters<typeof permission.ask>[0],
    ) {
      if (req.permission !== "edit") return req
      const guard = yield* memory.editGuard([...req.patterns]).pipe(Effect.orDie)
      if (guard.matches.length === 0) return req

      const matchedRules = guard.matches.map((match) => ({
        kind: match.kind,
        path: match.path,
      ...
      return {
        ...req,
        ruleset: Permission.merge(req.ruleset, guard.ruleset),
        metadata: {
          ...req.metadata,
          projectRules: matchedRules,
          projectRuleSeverity: guard.severity,
        },
      }
    })
      ...
      const context = (args: any, options: ToolExecutionOptions): Tool.Context => ({
        ...
        ask: (req) =>
          Effect.gen(function* () {
            ...
            const enriched = yield* withProjectEditRules(base)
            return yield* permission.ask(enriched).pipe(Effect.orDie)
          }),
      })
      ...
      function* (sessionID: SessionID) {
        ...
        while (true) {
          ...
          const outcome: "break" | "continue" = yield* Effect.gen(function* () {
            ...
            const [skills, env, memorySystem, instructions, projectMemory, modelMsgs] = yield* Effect.all([
              sys.skills(agent),
              sys.environment(model),
              sys.memory(),
              instruction.system().pipe(Effect.orDie),
              instruction.project().pipe(Effect.orDie),
              MessageV2.toModelMessagesEffect(msgs, model),
            ])
            const system = [...env, ...memorySystem, ...instructions, ...projectMemory, ...(skills ? [skills] : [])]
