...
function ProjectRuleBody(props: { request: PermissionRequest }) {
  const { theme } = useTheme()
  const matches = () => {
    const value = props.request.metadata?.projectRules
    return Array.isArray(value)
      ? value.filter(
          (item): item is { kind: string; path: string; matched: string; line: number; raw: string } =>
            !!item && typeof item === "object",
        )
      : []
  }
  const severity = () => {
    const value = props.request.metadata?.projectRuleSeverity
    return value === "do_not_edit" || value === "ask_before_edit" ? value : undefined
  }

  return (
    <Show when={matches().length > 0}>
      <box flexDirection="column" gap={1} paddingLeft={1} paddingBottom={1}>
        <text fg={severity() === "do_not_edit" ? theme.error : theme.warning}>
          {severity() === "do_not_edit"
            ? "Project rules say this file should not be edited without confirmation."
            : "Project rules say this file should be confirmed before editing."}
        </text>
        <For each={matches()}>
          {(rule) => (
            <text fg={theme.textMuted}>
              {`- line ${rule.line}: ${rule.raw.trim()} (matched ${normalizePath(rule.matched)})`}
            </text>
          )}
        </For>
      </box>
          ...
          const info = () => {
            ...
            if (permission === "edit") {
              ...
              return {
                ...
                body: (
                  <box flexDirection="column" gap={1}>
                    <ProjectRuleBody request={props.request} />
